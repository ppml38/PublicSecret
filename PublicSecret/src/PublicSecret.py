"""
What is this app?
This is a text editor cum stegnography tool that can be used to hide your secret messages inside punctuation marks.
If combined with cryptography, i.e, first encrypting with a cryptography algorithm before using this app to hide cipher behind punctuation marks, This tool will serve a great security purpose.
Why this app?
This app is kind of inspired from the book "Disappearing cryptography", where I found some techniques to hide data behind objects like image, audio etc.
and also behind grammers and sentence structures as contextless lingustic stegnography Ex. spammimic.com
But I did not came across 'hiding behind punctuations' allowing users to create a 'contextful' para, hence I created this app.
Since this app allows users to create a contextful para, this may escape, more intense scrutiny.
"""
from tkinter import *
...
def punct_highlight(fulltext):
    ...
    for line in fulltext.split('\n'):
        ...
        for m in iter:
            #print("punct "+str(count)+"."+str(m.span()[0])+","+str(count)+"."+str(m.span()[1]))
            if m[0] in [':','/']:
...
def scope_highlight(fulltext,reg,style):
    ...
    #if style=="scope" and re.search(reg,fulltext) is not None: #if scope, unscope all before scoping
    #    print("all unscope")
    #    stegno.tag_add("nonscope","1.0",END)
    scoped=False
    for m in iter:
        ...
        #print(style+"-("+start+","+stop+")")
        
        #print(stegno.tag_names(start))
        #print(stop.split(".")[0]+"."+str(int(stop.split(".")[1])-1))
        #print(stegno.tag_names(stop.split(".")[0]+"."+str(int(stop.split(".")[1])-1)))
...
def changecpr(event):
    ...
    #if changing: return
    #changing=True
    #Only text within : and / are considered for stegno
    #Text within / and : are ignored from stegno
    #cover=re.sub("/.*?:",'',''.join(re.findall(":.*/",stegno.get("1.0",END))))
    #cover=re.sub("/.*?:",'',''.join([''.join(a) for a in re.findall("(?:.*?:(.*)/.*)|(.*)",stegno.get("1.0",END),re.DOTALL)]),flags=re.DOTALL)
    #cover=cover[cover.find(':')+1:cover.rfind('/')] 
    #cover=re.sub("/.*?:",'',cover)
    #start="".join(re.findall("[,.;!?-](?!  )(?:(?<!  [,.;!?-])|(?<=[,.;!?-]  [,.;!?-]))",cover))
    ...
    if cprbkup.startswith(start):
        ...
        #messagebox.showerror("Stegno error","You have entered wrong puntuation. please correct it.")
        #stegno.insert(END,"  ")
    ...
    #scopelist=get_scope(fulltext,":(?!  )(?:(?<!  :)|(?<=[,.;!?:/-]  :)).*/(?!  )(?:(?<!  /)|(?<=[,.;!?:/-]  /))")


    #scope_highlight(fulltext,"/(?!  )(?:(?<!  /)|(?<=[,.;!?:/-]  /))[^/:]*?:(?!  )(?:(?<!  :)|(?<=[,.;!?:/-]  :))","nonscope")
    scope_highlight(fulltext,":(?!  )(?:(?<!  :)|(?<=[,.;!?:/-]  :)).*/(?!  )(?:(?<!  /)|(?<=[,.;!?:/-]  /))","scope")
    
    #stegno.edit_modified(0)
    #changing=False
...
def decode():
    ...
    #cover=stegno.get("1.0",END)    
    #cover=re.sub("/.*?:",'',''.join([''.join(a) for a in re.findall("(?:.*?:(.*)/.*)|(.*)",stegno.get("1.0",END),re.DOTALL)]),flags=re.DOTALL)
    #new="".join(re.findall("[,.;!?-](?!  )(?:(?<!  [,.;!?-])|(?<=[,.;!?-]  [,.;!?-]))",cover))
